...
Teleport [Nodes](../../architecture/nodes.mdx) have three primary modes of operation: *Auth*, *Proxy*, and *Node* (SSH) mode (in addition to more advanced modes like *Database*, *Apps*, *Trusted Cluster*, and *Kubernetes* mode). A single Node can run in up to all three modes. Optionally, you can configure a Node to act as the dedicated *Auth* or SSH Node for a cluster.
