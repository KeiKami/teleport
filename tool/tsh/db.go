...
func getMySQLCommand(db *tlsca.RouteToDatabase, cluster, user, name string) *exec.Cmd {
	args := []string{"--default-group-suffix", fmt.Sprintf("%v-%v", cluster, db.ServiceName)}
	if user != "" {
